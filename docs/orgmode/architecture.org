...
- Volume topdir is the longest matching mount point from =/proc/self/mounts=
  (btrfs multi-subvol safe; not pure =st_dev= parent walk alone).
- Default empty/list/restore/rm scan home trash plus existing trash on every
  non-pseudo mount (trash-cli multi-volume parity), including =/=.
