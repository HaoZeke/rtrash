...
page under =~/.local= from assets *embedded in the binary* (no source checkout).
Flags: ~--dry-run~, ~--force~ (after upgrades), ~--with-rm~, ~--prefix=DIR~.
Packagers: ~rtrash completions bash|zsh~, ~rtrash man~, or
